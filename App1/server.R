...
library(GISTools);library(devtools); library(shiny); library(maps); 
...
shinyServer(function(input, output) {
  ...
  ggmap(get_googlemap(center=c(lon = -93.920710, lat = 46.52), zoom=6, maptype="hybrid"))+
    geom_point(data=dd1[,c(1:3, 4+as.numeric(input$variable))], aes(x=Lon, y=Lat, 
                                                                    color=dd1[,input$variable]), size=8.5)+
    geom_text(data=dd1[,c(1:3, 4+as.numeric(input$variable))], aes(x=Lon+c(0.8,-0.2,0.8,0.8,1.2,0.9,1.5,0.8,0.8),
                                          y=Lat+c(0.2, -0.2, -0.1, 0.3, -0.2, -0.2, 0.2, -0.25, -0.25),
                                          label=Site2), size=7,  fontface="bold")+
    scale_color_gradient2(low = "red", mid = "white", high = "blue", 
                          midpoint = mean(subset(pdat, Trt==input$variable)$TotalBiomass, na.rm=T), space = "rgb", na.value = "grey50", guide = "colourbar")
